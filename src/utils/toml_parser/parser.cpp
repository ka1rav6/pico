...
Table *Parser::find_or_create_child(Table *parent, const std::string &name,
                                    bool is_array) {
    for (Table *child : parent->children) {
...
bool Parser::parse_value(Value *out) {
    ...
    case TokenType::TOK_LBRACE: {
        Table *tbl = new Table;
        if (!parse_inline_table_content(tbl)) {
            delete tbl;
            return false;
        }
...
bool Parser::parse_array(Value *out) {
...
bool Parser::parse_inline_table_content(Table *out) {
...
bool Parser::parse_entry(Entry *out) {
    Key k;
    if (!parse_key(&k))
        return false;
